...
export default async function ReferencePage({ params }: Params) {
  ...
  return (
    <>
      <PageHeader
        eyebrow={meta.client ?? copy.clientAnonyme}
        titre={meta.title}
        intro={meta.excerpt}
        breadcrumbs={fil}
      />
      ...
      <Section>
        ...
        {meta.chiffres.length > 0 ? (
          <dl className="mb-14 grid gap-px border border-steel/30 bg-steel/30 sm:grid-cols-3">
            {meta.chiffres.map((chiffre) => (
              <div key={chiffre.label} className="bg-frost p-7">
