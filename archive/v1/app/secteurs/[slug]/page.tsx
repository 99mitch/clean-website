...
export default async function SecteurPage({ params }: Params) {
  ...
  return (
    <>
      ...
      <Section>
        <div className="grid gap-14 lg:grid-cols-12">
          ...
          <aside className="lg:col-span-5">
            <Reveal className="rounded border border-steel/30 bg-frost p-7">
      ...
      {services.length > 0 ? (
        <Section tone="frost" labelledBy="services-secteur">
          <SectionHeader
            id="services-secteur"
      ...
      {meta.faq.length > 0 ? (
        <Section labelledBy="faq-secteur">
          <SectionHeader id="faq-secteur" title={secteurPage.faqTitre} />
          <dl className="mt-12 max-w-[68ch] divide-y divide-steel/30 border-y border-steel/30">
            {meta.faq.map((item) => (
