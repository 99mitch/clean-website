    ...
    <div>
      ...
      >
        <p className="eyebrow mb-6 text-slate">
          <span className="text-signal">{stepCopy.numero}</span>
        ...
        {step === 5 ? (
          <form onSubmit={onSubmit} noValidate>
            ...
            {submitError ? (
              <div
                role="alert"
                className="mt-8 max-w-2xl border-l-2 border-[#B3261E] bg-[#B3261E]/5 p-5"
              >
...
function StepProgress({
  ...
  return (
    <nav aria-label={copy.progression.label}>
      <ol className="grid list-none grid-cols-5 gap-x-2">
          ...
          return (
            <li key={stepCopy.numero}>
              <button
                ...
                className={`inline-flex min-h-11 w-full items-center justify-center border-t-4 pt-2 font-mono text-13 tracking-[0.12em] transition-colors duration-150 ${
                  isCurrent
                    ? 'border-signal text-ink'
                    : reachable
                      ? 'border-ink/70 text-slate hover:text-ink'
                      : 'border-graphite/40 text-graphite'
...
function Recap({
  ...
  return (
    <div className="mt-10 max-w-2xl border-t-2 border-ink pt-5">
        ...
        {lignes.map((ligne) => (
          <div key={ligne.step} className="flex flex-wrap items-baseline gap-x-3">
            ...
            <button
              ...
              className="ml-auto min-h-11 text-13 text-cobalt underline underline-offset-4 hover:text-signal"
            >
              {copy.recap.modifier}
            </button>
          </div>
