...
export default async function QuiSommesNousPage() {
  ...
  return (
    <>
        ...
        <div className="grid gap-x-6 gap-y-14 sm:grid-cols-2">
          {copy.sections.map((section, index) => (
            <Reveal key={section.titre} delay={index * 60}>
              <p className="font-mono text-13 text-azure">
      ...
      <Section tone="frost" labelledBy="equipe-titre">
        <SectionHeader
          id="equipe-titre"
          ...
          <ul className="mt-12 grid list-none gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {equipe.map((membre, index) => (
              <Reveal
                ...
                delay={index * 60}
                className="rounded border border-steel/30 bg-white p-7"
              >
                ...
                {membre.meta.citation ? (
                  <blockquote className="mt-4 border-l-2 border-azure pl-4 text-15 text-slate">
                    {membre.meta.citation}
                  </blockquote>
                ) : null}
              </Reveal>
